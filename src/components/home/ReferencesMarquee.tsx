...
function LogoItem({ name, logo }: { name: string; logo: string | null }) {
  return (
    <div className="logo-item flex items-center justify-center mx-10 shrink-0 w-48 h-20 cursor-default">
      {logo ? (
        <>
          <img
            src={logo}
          ...
          />
          <span className="hidden font-display font-bold text-xs uppercase tracking-widest text-brand-dark/25 hover:text-brand-red transition-colors">
            {name}
          </span>
        </>
      ) : (
        <span className="font-display font-bold text-xs uppercase tracking-widest text-brand-dark/25 hover:text-brand-red transition-colors">
          {name}
        </span>
      )}
    </div>
      ...
      <div className="container-wide mb-12">
        <ScrollAnimator>
          <div className="animate-on-scroll flex flex-col md:flex-row items-start md:items-end justify-between gap-4">
            ...
            <p className="text-brand-dark/40 text-sm max-w-xs">
